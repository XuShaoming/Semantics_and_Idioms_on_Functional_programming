  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([v1 (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? v1)
               (let ([fname (fun-nameopt (closure-fun v1))]
                     [fformal (fun-formal (closure-fun v1))]
                     [fbd (fun-body (closure-fun v1))]
                     [cenv (closure-env v1)])
                 (if (false? fname)
                     (eval-under-env fbd (cons (cons fformal v2) cenv))
                     (eval-under-env fbd (cons (cons fname v1) (cons (cons fformal v2) cenv)))))
        ...
        [(fst? e)
         (let ([v1 (eval-under-env (fst-e e) env)])
           (if (apair? v1)
               (apair-e1 v1)
               (error (format "MUPL fst applied to non-pair"))))]
        [(snd? e)
         (let ([v1 (eval-under-env (snd-e e) env)])
           (if (apair? v1)
               (apair-e2 v1)
               (error (format "MUPL snd applied to non-pair"))))]
        [(isaunit? e)
